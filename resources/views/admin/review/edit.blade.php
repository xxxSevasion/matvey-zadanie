...
<x-app-layout>
    <x-slot:header>
        <div class="w-full flex items-center justify-between">
            ...
            <a href="{{ route('review.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center">{{ __('Посмотреть все заявки') }}</a>
        </div>
        </x-slot>
        <div class="my-4 p-4 bg-white max-w-3xl mx-auto rounded-md">

            <x-splade-form method="PUT" action="{{ route('review.update', $review->id) }}" :default="$review">

                <x-splade-input name="name"  label="{{ __('Название отзыва') }}" placeholder="{{ __('Есть пробитие!') }}"/>
